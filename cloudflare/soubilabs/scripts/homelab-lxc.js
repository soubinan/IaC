...
async function downloadLatestVersion(latestBuildId) {
  const app_name = latestBuildId.replace(/-latest.*/, "");
  const latestBuildGQL = {
    query: `
      query latestBuild {
        application(where: {name: "${app_name}"}) {
          builds(orderBy: publishedAt_DESC, first: 1) {
            buildId
          }
        }
      }
  ...
  const latestBuildRequest = {
    body: JSON.stringify(latestBuildGQL),
    method: "POST",
    headers: {
      "content-type": "application/json;charset=UTF-8",
      "Accept": "application/json",
  ...
  const latestBuildResponse = await fetch(url, latestBuildRequest);
  const latestBuildJsonData = await gatherResponse(latestBuildResponse);
  ...
  if (reqUrl.pathname.includes("/downloads")) {
    ...
    buildsByAppsJsonData.data.applications.forEach(app => {
      app.builds.forEach((build, index) => {
        ...
        if (index === 0) {
          let regex = new RegExp(`(${app.name})-(.+)-(${build.architecture})`, "g");
          let latest = build.buildId.replace(regex, "$1-latest-$3");
          let version = build.buildId.replace(regex, "$2");

          const latestMeta = `${latest}-meta`;
          const latestRoot = `${latest}-root`;
          ...
          <td>${latest} (v${version})</td>
          <td><a href="/downloads/${latestRoot}">${latestRoot}</a> | <a href="/downloads/${latestMeta}">metadata</a></td>
    ...
    <div id="container">
      ...
      <article id="intro">
        ...
        <br>
        <i>
        At most the 03 last versions and at least only the latest version are listed for each application's build.<br>
        Use the links following the pattern [app_name]-latest-[arch]-root|meta.tar.gz to always download the latest version of the image.<br>
        </i>
        </p>
